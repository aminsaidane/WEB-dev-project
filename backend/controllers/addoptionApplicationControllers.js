...
module.exports.getCurrentUserApplications = async (req, res) => {
  try {
...
module.exports.editApplicationStatus = async (req, res) => {
  try {
    const { id } = req.params;
    const { status } = req.body; // Only status is updated

    const updatedApplication = await AddoptionApplication.findByIdAndUpdate(
      id,
      { status },
      { new: true }
    );
